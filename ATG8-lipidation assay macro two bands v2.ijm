...
//Loop analysis through the list of .scn files
	...
	//Wait for the user to adjust the ROIs size and position
		waitForUser("Add all ROIs to ROI manager, then hit OK.\n\n1. For each lane select first the GFP-ATG8 band and then the GFP-ATG8-PE band.\n\n2. Add two ROIs selecting background for GFP-ATG8 and GFP-ATG8-PE\n\n3. NB! Keep ROI size the same for all selections!\n\n3. Hit ok, when done! "); 
	//Rename the ROIs and save them
			...
			for ( r=0; r<n; r++ ) {
				 roiManager("Select", r);
				  odd = r % 2;		
					if (!odd) {
						roiManager("Rename", "Tag-ATG8 fusion sample " + x);
						} else {
						roiManager("Rename", "Tag-ATG8-PE sample " + x-1);
						x=x-1;
						}
						x=x+1;
					}
			...
			//create a column for Integrated density without background
				current_last_row = Table.size("Assay Results");
				Background_for_Tag_ATG8 = Table.get("RawIntDen", current_last_row-2, "Assay Results");
				Background_for_Tag_ATG8_PE = Table.get("RawIntDen", current_last_row-1, "Assay Results");
			...
			//create a column with calculation for the Tag-ATG8-PE, expressed as % of all tagged protein detected in the sample
				...
				for (row = 1; row < current_last_row; row++) {
					Total_Signal = (Table.get("RawIntDen_without_background", row-1, "Assay Results")) + (Table.get("RawIntDen_without_background", row, "Assay Results"));
					Tag_ATG8_PE = Table.get("RawIntDen_without_background", row, "Assay Results");
					Tag_ATG8_PE_percent = 100*Tag_ATG8_PE/Total_Signal;
					Table.set("Tag-ATG8-PE as % of cumulative tag signal detected in the sample", row, Tag_ATG8_PE_percent, "Assay Results");
					row = row+1;
				 }
